...
const BulkImportModal: React.FC<BulkImportModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleConfirmImport = () => {
        if (validLeads.length > 0) {
            dispatch({ type: 'ADD_BULK_LEADS', payload: validLeads });
            setStep('done');
        }
    }
    ...
    const renderInitialStep = () => (
        <div className="space-y-6">
            <div>
                ...
                <Button onClick={generateTemplate} variant="secondary" className="mt-3">
    ...
    const renderPreviewStep = () => (
         <div className="space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-center pt-4 border-t dark:border-gray-700">
                ...
                <Button onClick={handleConfirmImport} disabled={validLeads.length === 0}>
                    {`Importar ${validLeads.length} Prospecto(s)`}
                </Button>
            </div>
        </div>
    );
