...
const Users: React.FC = () => {
    ...
    const handleAddRole = (e: React.FormEvent) => {
        e.preventDefault();
        const potentialRole = newRole.trim() as UserRole;
        if (potentialRole && !roles.includes(potentialRole)) {
            dispatch({ type: 'ADD_ROLE', payload: potentialRole });
            setNewRole('');
        } else {
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            {users.map((user: User) => <UserRow key={user.id} user={user} onEdit={handleEditClick} onResetPassword={handleOpenResetModal} />)}
                        </tbody>
                    </table>
                </div>
            </div>
